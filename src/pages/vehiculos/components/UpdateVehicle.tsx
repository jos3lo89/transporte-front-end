...
type Props = { vehiculo: VehiculosApi; traerVehiculos: () => Promise<void> };

const UpdateVehicle = ({ vehiculo, traerVehiculos }: Props) => {
  const [openModal, setOpenModal] = useState(false);
  const [vehicleData] = useState<VehiculosApi>(vehiculo);
  ...
  const onSubmit = async (values: FormSchemetype) => {
    ...
    try {
      const res = await axios.put(
        `/vehiculos/change-state/${vehicleData.id}`,
        values
      );
      console.log(res);
      setOpenModal(false);
          ...
          <div className="flex gap-4">
            <div className="mb-3">
              <Controller
                name="state"
                control={control}
                defaultValue={vehicleData.estado}
                render={({ field }) => (
